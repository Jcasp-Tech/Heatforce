...
const BackgroundBubblesMobile=()=>{
    function Box(props: JSX.IntrinsicElements['mesh']) { const ref = useRef<THREE.Mesh>(null!)
        useFrame(() => (ref.current.position.x < 10 ? ref.current.position.x += 0.006 : ref.current.position.x = ref.current.position.x - 20))
    ...
    return (
        ...
        <Canvas  style={{backgroundColor: '#e6f0f9'}}>
          ...
          {[...Array(1500)].map((_, i) => (
            <Box key={i} position={[(Math.random() - 0.5) * 25, (Math.random() - 0.5) * 7, 0 ]} scale={(Math.random() * 0.4) - 0.2} />
          ))}
          </Canvas>
      </div>
    )
}
export default BackgroundBubblesMobile;