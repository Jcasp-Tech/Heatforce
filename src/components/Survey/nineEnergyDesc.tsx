...
export interface NineEnergyUseProps {
...
const NineEnergyUse = (props: NineEnergyUseProps) => {
  const { handleChange, isMobile, setBack, back, allQuestionsData } = props;
  const questionConst = QUESTION
  const [giveAnswer, setGiveAnswer] = useState(allQuestionsData?.no_energy_usage);
  const [isAnswered, setIsAnswered] = useState<Boolean>(allQuestionsData?.no_energy_usage !== DEFAULTDATA?.no_energy_usage);
  ...
  useEffect(() => {
    ...
    if (!isAnswered) {
      if (giveAnswer !== '') {
        handleChange(questionIndex + 1, { no_energy_usage: giveAnswer }, questionConst[questionIndex + 1].id);
      }
    }
  ...
  return (
    <div data-aos={
      ...
      <div>
        <div className={`${styles.backButtonDiv} mobile-view`} onClick={() => handleChange(questionIndex - 2, {}, questionConst[questionIndex - 2].id)}
        >
          <div>
            <div  >
      ...
      <div className={`${styles.infoWrapper}`}>
        <InfoDiv content={'Try to give an accurate estimate of your consumption. If you have high usage appliances or a hot tub, for instance, you would be classed as high'} />
      </div>
    </div>
  )
}

export default NineEnergyUse